...
Az $m$ tömegű teher kényszeríti a rugót a megnyúlásra, ennek hatására ébred az $\vec{F_r}$ rugóerő.
\begin{equation}
	m  \vec{g} = \vec{F_r} 
\end{equation}
\begin{equation}
	F_r = \frac{\Delta h}{{c_r}}
\end{equation}
\begin{equation}
\Delta h = {c_r}  m  g = \SI{49}{\milli\meter}
\end{equation}

\subsubsection*{2. Impulzuserők vizsgálata}
...
\begin{figure}[h]
	...
	\draw [dash pattern={on 8mm off 1mm on 0.5mm off 1mm}] (2,-1) -- (2,-8);
	
	%impulzus erők
	\draw [ultra thick][->](2,-5) -- (2,-6) node[anchor=east]{$\dot{m}$} node[anchor=west]{$\vec{J_{BE}}$};
	\draw [ultra thick][->](0.72,-5.75) -- ++ (60:-1) node[anchor=east]{$\dfrac{1}{2}\dot{m}$} node[anchor=west]{$\vec{J_{KI1}}$} ;
	\draw [ultra thick][->](3.28,-5.75) -- ++ (120:-1) node[anchor=east]{$\dfrac{1}{2}\dot{m}$} node[anchor=west]{$\vec{J_{KI2}}$} ;
	...
	\node at (1.3,-2) {$\vec{J_{BE}}$};
	\draw [ultra thick][->] (1.8,-2*2) -- ++ (240: 1.15*2);
	\draw [ultra thick][<-] (1.8,-4*2) -- ++ (120: 1.15*2);
	\node at (0.9,-4.5) {$\vec{J_{KI1}}$};
	\node at (0.9,-7.25) {$\vec{J_{BE}}$};
